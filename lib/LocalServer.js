...
LocalServer.prototype._connectService = function (clientSocket, options) {
  ...
  serviceSocket.on('connect', function () {
    serviceSocket.on('close', function (had_error) {
      this._emitServiceClose(had_error, serviceSocket)
    })
    serviceSocket.pipe(clientSocket).pipe(serviceSocket)
    this._emitServiceRedirection(clientSocket, serviceSocket)
  })
 ...
 * @param localPort {number} - Server port at local
 * @param client {Socket} - Client connection socket
 * @param service {Socket} - Service connection socket
 */
LocalServer.prototype._emitServiceRedirection = function (clientSocket, serviceSocket) {
  ...
  // TODO: document service-redirection-dynamic and service-redirection-fixed events
...
LocalServer.prototype._onConnection = function (clientSocket) {
  ...
  clientSocket.on('close', function (had_error) {
    this._emitClientClose(had_error, clientSocket)
  })

  isDynamicServer = this.isDynamicServer
  serviceHost = this.serviceHost(clientSocket.remoteAddress)
...
// TODO: Add Socket external reference, and document it at code
